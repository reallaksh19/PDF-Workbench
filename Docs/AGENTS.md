...
```
RULE:  Phase 0–1 may import PDF.js, Fabric.js, Tesseract.js directly in
...
TEST:  core/annotations/types.ts must not import fabric.\*
       core/ocr/types.ts must not import pdfjs-dist.\*
```

`PdfAnnotation` holds `rect: Rect`, not `fabricObject: fabric.Object`.
...
Adapters call `canvas.getContext`, `fabric.Canvas()`, `MathJax.typesetPromise`.
...
AnnotationOverlay    calls FabricAdapter (Phase 3+)
...
|Store fabric.Object in Zustand|INV-4|
...
□ All new domain types free of library references?    (must be YES)
